...
def main():
    ...
    <div class="main-header">
            <h1>📋 Bitwave Form 8949 Generator (Jatin - TEST)</h1>
    ...
    with st.expander("📖 Instructions & Bitwave Requirements", expanded=False):
        ...
        - "2023-12-31 22:01:13 UTC" → "2023-12-31 22:01:13" (UTC removed)
        - Will fallback to timestampSEC only if timestamp column is missing/invalid
        ...
        - ✅ Flexible timestamp format handling with UTC cleanup
        """)
    ...
    if uploaded_file is not None:
        try:
            ...
            # FIXED: Check for timestamp columns with PRIORITY for 'timestamp' column
            timestamp_column = None
            # Priority order: timestamp (Column D) first, then fallbacks
            for col in ['timestamp', 'timestampSEC', 'date', 'saleDate']:
...
def parse_flexible_timestamp(timestamp_value):
    """FIXED: Parse timestamp in various formats and properly handle UTC removal"""
    ...
    # Handle Unix timestamp (numeric) - fallback option
    try:
    ...
    # FIXED: Handle string timestamps (PRIMARY LOGIC FOR 'timestamp' COLUMN)
    try:
        # Remove UTC suffix if present - this is the key fix
...
def process_bitwave_transactions_fixed(df, tax_year, proceeds_column, exchange_rate_column, timestamp_column):
    """FIXED: Process Bitwave actions report with proper timestamp column priority"""
    transactions = []
    validation_warnings = []
    ...
    # FIXED: Show which column we're using with priority indication
    if timestamp_column == 'timestamp':
        st.success(f"🕐 Using '{timestamp_column}' column (Column D - PRIMARY) for sale dates")
    else:
        st.info(f"🕐 Using '{timestamp_column}' column for sale dates (fallback - 'timestamp' column not found)")
    ...
    for _, row in sell_actions.iterrows():
        try:
            # FIXED: Extract and validate sale date using improved timestamp parsing
            try:
